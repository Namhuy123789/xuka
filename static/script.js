...
// Hàm khóa phím để ngăn gian lận trong chế độ thi
...
// Hàm mở khóa phím sau khi thi xong
...
// Hàm kích hoạt ngắt mạng bằng Service Worker
  ...
  if ('serviceWorker' in navigator) {
    try {
      if (location.protocol !== 'https:' && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
        ...
        alert('Không thể ngắt mạng: Yêu cầu HTTPS hoặc localhost. Vui lòng kiểm tra kết nối thủ công.');
        return;
      }
      ...
      alert(`Không thể ngắt mạng: ${err.message}. Vui lòng kiểm tra kết nối mạng và thử lại.`);
    }
  } else {
    console.error('[ERROR] Trình duyệt không hỗ trợ Service Worker');
    alert('Trình duyệt không hỗ trợ Service Worker. Không thể ngắt mạng.');
  }
}

// Hàm vô hiệu hóa Service Worker
...
async function startQrScanner() {
  ...
  try {
    ...
    devices = list || [];
    if (devices.length === 0) {
      qs('#qr-error').textContent = 'Không tìm thấy camera trên thiết bị! Vui lòng kiểm tra thiết bị hoặc kết nối camera ngoài.';
      qs('#qr-error').classList.remove('hidden');
      return;
    }
    ...
    qs('#qr-error').textContent = `Lỗi camera: ${err?.message || 'Không thể khởi động camera. Vui lòng kiểm tra thiết bị và thử lại.'}`;
    qs('#qr-error').classList.remove('hidden');
  }
}
...
qs('#flip-camera')?.addEventListener('click', async () => {
  ...
  try {
    ...
    qs('#qr-error').textContent = 'Không thể chuyển camera! Vui lòng kiểm tra thiết bị camera.';
    qs('#qr-error').classList.remove('hidden');
  }
});
...
qrFileInput?.addEventListener('change', async (e) => {
  ...
  if (!file) {
    qs('#qr-error').textContent = 'Không có tệp nào được chọn! Vui lòng chọn một tệp hình ảnh.';
    qs('#qr-error').classList.remove('hidden');
    return;
  }
  ...
  try {
    ...
    qs('#qr-error').textContent = `Không thể đọc mã QR từ ảnh: ${err.message || 'Lỗi không xác định. Vui lòng thử lại với hình ảnh khác.'}`;
    qs('#qr-error').classList.remove('hidden');
  } finally {
    qrFileInput.value = '';
  }
...
async function verifyAndLogin(qrText) {
  qs('#qr-error').classList.add('hidden');
  if (!qrText) {
    qs('#qr-error').textContent = 'Mã QR rỗng! Vui lòng quét lại hoặc tải lên mã QR hợp lệ.';
    qs('#qr-error').classList.remove('hidden');
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(errorData.msg || `Lỗi máy chủ: ${res.status} ${res.statusText}. Vui lòng thử lại sau.`);
    }
    ...
    if (data.status === 'success') {
      ...
      throw new Error(data.msg || 'Mã QR không hợp lệ! Vui lòng kiểm tra mã QR và thử lại.');
    }
  } catch (err) {
    console.error('[ERROR] Lỗi verifyAndLogin:', err);
    qs('#qr-error').textContent = err.message || 'Lỗi kết nối máy chủ! Vui lòng kiểm tra kết nối mạng và thử lại.';
    qs('#qr-error').classList.remove('hidden');
  }
}
...
qs('#account-login-form')?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      throw new Error(data.msg || 'Sai tên đăng nhập hoặc mật khẩu! Vui lòng kiểm tra lại.');
    }
  } catch (err) {
    const x = qs('#login-error');
...
async function loadExamCodes() {
  ...
  try {
    ...
    p.textContent = 'Không thể tải danh sách mã đề. Vui lòng kiểm tra kết nối mạng và thử lại!';
    select.parentElement.appendChild(p);
  }
}

// Bắt đầu bài thi với kiểm tra đầu vào
...
async function startExam() {
  ...
  // Kiểm tra định dạng SBD (giả định là chuỗi chữ số)
  if (!/^\d+$/.test(sbd)) {
  ...
  if (isNaN(dobDate.getTime()) || dobDate > new Date()) {
    formError.textContent = 'Ngày sinh không hợp lệ! Vui lòng chọn ngày hợp lệ.';
    formError.classList.remove('hidden');
    return;
  }
  if (!name || !sbd || !dob || !made) {
    formError.textContent = 'Vui lòng nhập đầy đủ Họ tên, Số báo danh, Ngày sinh và Mã đề!';
    formError.classList.remove('hidden');
    return;
  }
  ...
  try {
    ...
    const duration = Number(data?.duration_sec || 3600); // Lấy thời gian từ server
  ...
  try {
    ...
    try {
      ...
      alert(`Không thể tải câu hỏi: ${e.message}. Vui lòng kiểm tra kết nối mạng hoặc liên hệ quản trị viên.`);
    }
  }
}
...
// Xử lý nội dung toán học (hàm này chuyển đổi các biểu thức toán học thành cú pháp MathJax)
...
// Xử lý nội dung vật lý (hàm này chuyển đổi các ký hiệu vật lý thành cú pháp MathJax)
...
// Xử lý nội dung hóa học (hàm này chuyển đổi công thức hóa học thành cú pháp MathJax với \ce)
...
// Xử lý toàn bộ nội dung câu hỏi (kết hợp toán, vật lý, hóa học)
...
// Kiểm tra xem nội dung có nằm trong biểu thức toán học không
