...
def save_result():
    try:
        ...
        for a in answers:
            ...
            if kieu == "tu_luan":
				# Lấy câu trả lời học sinh, đảm bảo luôn là string
				tra_loi = a.get("tra_loi_hoc_sinh")  # có thể None
				if tra_loi is None or str(tra_loi).strip() == "":
					tra_loi = "[Chưa trả lời]"
				else:
					tra_loi = str(tra_loi).strip()

				goi_y = a.get("goi_y_dap_an")
				if goi_y is None:
					goi_y = ""
